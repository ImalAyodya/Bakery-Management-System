...
//search end ,then add values change the read
    ...
    axios.post('http://localhost:8000/deliveryorder/save', { OrderDelivery: newDelivery })
      .then(response => {
        if (response.data.success) {
          ...
          setNewDelivery({ orderid: '', orderlocation: '', assigneddriver: '', diliveryvehicle: '', diliverydate: '', diliverystatus: '' });
...
//delete ekaa
  ...
  const orderStatusData = [40, 35, 25];
  ...
  return (
    <div>
      ...
      <div className="search-container">
        <input type="text" placeholder="Search by Order ID" className="search-input" value={searchTerm} 
    onChange={handleSearchChange}  />
          ...
          <tbody>
            {filteredOrders.map((delivery) => (
              <tr key={delivery._id}>
                ...
                <td>
                  <button className="delete-button" onClick={() => handleUpdateClick(delivery._id)}>Update</button><br></br><br></br>
      ...
      <animated.div style={formAnimation} className="form-popup2">
        <form className="form2" onSubmit={handleSubmit}>
          <h1>New Order Delivery</h1>
          <div className="form-group2">
            <label htmlFor="orderId">Order ID:</label>
            <input type="text" id="orderId" name="orderid" value={newDelivery.orderid} onChange={handleInputChange} required />
          </div>
          <div className="form-group2">
            <label htmlFor="orderLocation">Order Location:</label>
            <input type="text" id="orderLocation" name="orderlocation" value={newDelivery.orderlocation} onChange={handleInputChange} required />
          </div>
          <div className="form-group2">
      ...
      {showSummary && (
        <div className="summary-popup">
          <OrderSummary orderStatusData={orderStatusData} />
          <button className="close-summary-button" onClick={handleCloseSummary}>Close</button>
        </div>
      )}
    </div>
  );
...
const handleGenerateReport = () => {
  ...
  if (filteredOrdersByDate.length === 0) {
    ...
    doc.autoTable({
      head: [['Order ID', 'Location', 'Driver', 'Vehicle', 'Status']],
      body: tableData,
      startY: 30,
      theme: 'grid',
    });
    ...
    <div>
          ...
          <tbody>
            {filteredOrders.map((delivery) => (
              <tr key={delivery._id}>
                ...
                <td><b>{delivery.OrderDelivery?.diliverystatus}</b></td>
                <td>
                  <button className="update-button" onClick={() => handleUpdateClick(delivery._id)}>Update</button><br /><br />
                  <button className="delete-button" onClick={() => handleDelete(delivery._id)}> Delete </button>
                </td>
              </tr>
            ))}
      ...
      <animated.div style={formAnimation} className="form-popup2">
        <form className="form2" onSubmit={handleSubmit}>
          ...
          <button className="das-submit-button" type="submit">Submit</button>
        </form>
      </animated.div>
      {addDelivery && (
        <div className="modal-overlay">
      ...
      {showSummary && (
        <div className="order-summary-modal">
          <div className="order-summary-content">
            ...
            <button className="close-summary-button" onClick={handleCloseSummary}>
              Close Summary
            </button>
            </div>
          </div>
